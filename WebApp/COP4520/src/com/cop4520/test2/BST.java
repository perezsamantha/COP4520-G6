...
public class BST {
	...
	public BST () { // initialize tree
		AtomicStampedReference<Node> initLeft = new AtomicStampedReference<Node>(new Node(Node.DUMMY2, -1, null), 0);
		AtomicStampedReference<Node> initRight = new AtomicStampedReference<Node>(new Node(Node.DUMMY1, -1, null), 0);
		root = new AtomicStampedReference<Node>(new Node(Node.DUMMY1, -1, null, initLeft, initRight), 0);
		root.getReference().left.getReference().left = new AtomicStampedReference<Node>(new Node (Node.DUMMY3, -1, null), 0);
	...
	public Boolean insert(int key, int value, RemoteClient remoteClient) {
		while (true) {
			SeekRecord sr = seek(key);
			if (sr.leaf.getReference().key != key) {
				...
				AtomicStampedReference<Node> newLeaf = new AtomicStampedReference<Node>(new Node(key, value, remoteClient), 0);
				...
				AtomicStampedReference<Node> newSibling = new AtomicStampedReference<Node>(new Node(currKey, currValue, remoteClient), 0);

				if (key < currKey) {
					newInternal = new AtomicStampedReference<Node>(new Node(currKey, currValue, remoteClient, newLeaf, newSibling), 0);
				} else if (key > currKey) {
					newInternal = new AtomicStampedReference<Node>(new Node(key, value, remoteClient, newSibling, newLeaf), 0);
				} else {
					return false;
				}
